...
async function fetchProductos(spreadsheetId?: string, sheetName?: string) {
  const id = spreadsheetId || process.env.SPREADSHEET_ID;
  if (!id) {
    console.error('Setea la variable de entorno SPREADSHEET_ID con el id del spreadsheet o pásalo como argumento.');
    process.exit(1);
  }

  const sName = sheetName || 'PRODUCTOS';
  ...
  const range = `${sName}!A1:Z`;
  console.log(`🔎 Leyendo spreadsheetId=${id} range=${range}`);
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    for (let c = 0; c < keys.length; c++) {
      ...
      } else if (key.includes('precio') || key.includes('costo') || key.includes('precio') || key.includes('stock') || key.includes('cantidad')) {
  ...
  const outPath = path.resolve(outDir, 'productos.json');
...
if (require.main === module) {
  ...
  const argSheet = process.argv[3];
  fetchProductos(argId, argSheet).catch((err) => {
