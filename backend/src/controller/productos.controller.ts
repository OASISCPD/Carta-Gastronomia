...
import {
  insertProductoInSheet,
  updateProductoInSheet,
  updateProductoImageUrl,
...
import { enqueueSheetsWrite } from "../services/sheetsQueue";
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Campos requeridos: nombre, id_categoria, tipo, precio_venta, costo",
      });
    }
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Debe proporcionar al menos un campo para actualizar: estado, precio_venta, costo",
      });
    }
    ...
    // 1. Actualizar en la base de datos local (rápido, ~50ms)
    const dbFields: string[] = [];
    const dbValues: any[] = [];
    if (updates.estado !== undefined) {
      dbFields.push("estado = ?");
      dbValues.push(updates.estado);
    }
    if (updates.precio_venta !== undefined) {
      dbFields.push("precio_venta = ?");
      dbValues.push(updates.precio_venta);
    }
    if (updates.costo !== undefined) {
      dbFields.push("costo = ?");
      dbValues.push(updates.costo);
    }
    ...
    if (dbFields.length > 0) {
      ...
      const [dbResult] = await pool.query(
        `UPDATE productos SET ${dbFields.join(", ")}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
        dbValues,
      );
      dbUpdated = (dbResult as any).affectedRows > 0;
    }

    if (!dbUpdated) {
      return res.status(404).json({
        success: false,
        message: `Producto con ID ${id} no encontrado en la base de datos`,
      });
    }

    // 2. Actualizar en Google Sheets via queue (con reintentos automáticos)
    enqueueSheetsWrite(`update-${id}`, () =>
      updateProductoInSheet(id, updates),
    );
...
export const uploadProductoImage = async (req: Request, res: Response) => {
  try {
    ...
    if (!file) {
      return res
        .status(400)
        .json({ success: false, message: "Imagen requerida (field: image)" });
    }
    ...
    if (!allowedMimes.includes(file.mimetype)) {
      return res.status(400).json({
        success: false,
        message: `Tipo no permitido: ${file.mimetype}. Solo jpeg, png, webp, gif.`,
      });
    }
    ...
    try {
      ...
      console.warn(
        "[UPLOAD IMAGE] No se pudo actualizar Google Sheets:",
        sheetErr?.message,
      );
    }
...
export const syncProductosFromSheet = async (req: Request, res: Response) => {
  try {
    const safeBody =
      req && typeof req.body === "object" ? (req.body as any) : {};
    const spreadsheetId =
      safeBody.spreadsheetId ||
      req.query?.spreadsheetId ||
      process.env.SPREADSHEET_ID_PRODUCTOS ||
      process.env.SPREADSHEET_ID;
    const sheetName = safeBody.sheetName || req.query?.sheetName || "PRODUCTOS";

    if (!spreadsheetId) {
      console.error(
        "[SYNC PRODUCTOS] Missing spreadsheetId and no SPREADSHEET_ID in env",
      );
      return res.status(400).json({
        success: false,
        message:
          "spreadsheetId required (body or query) or set SPREADSHEET_ID in .env",
      });
    }
    ...
    if (!rows || !Array.isArray(rows))
      return res
        .status(500)
        .json({ success: false, message: "No se obtuvieron filas" });
    ...
    for (const r of rows) {
      const producto: any = {
        ...
        id_categoria:
          r.id_categoria != null ? String(r.id_categoria).trim() : "",
        tipo: r.tipo || "Simple",
        ...
        url_image:
          r.url_image || r.imagen_url || r.image || r.image_url || null,
      };
    ...
    return res.json({
      success: true,
      message: "Sync productos completed",
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error on sync", error: err?.message });
  }
};
...
export const syncProductosFromJson = async (req: Request, res: Response) => {
  try {
    ...
    return res.json({
      success: true,
      message: "Sync productos from JSON completed",
      result: { inserted, updated },
    });
  } catch (err: any) {
