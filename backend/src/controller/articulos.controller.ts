...
import { bulkUpsertArticulosService, upsertArticuloByKeyService } from '../services/articulos.service';
...
export const getArticulos = async (req: Request, res: Response) => {
  try {
    ...
    if (disponibilidad) filters.disponibilidad = disponibilidad as 'HABILITADO' | 'DESHABILITADO';
    ...
    return res.status(500).json({ success: false, message: 'Error al obtener artículos', error: error?.message });
  }
};
...
export const syncArticulos = async (req: Request, res: Response) => {
  try {
    ...
    if (!rows || !Array.isArray(rows)) {
      return res.status(500).json({ success: false, message: 'No se obtuvieron datos desde Sheets' });
    }
    ...
    const mapped = rows.map((item: any) => ({
      ...
      fecha_alta_producto: toMySQLDateTimeArgentina(item.fecha_alta_producto) || null,
      fecha_actualizacion_costo: toMySQLDateTimeArgentina(item.fecha_actualizacion_costo) || null,
      fecha_actualizacion_producto: toMySQLDateTimeArgentina(item.fecha_actualizacion_producto) || null,
    ...
    return res.status(500).json({ success: false, message: 'Error al sincronizar', error: error?.message });
  }
};
/**
...
export const getArticuloById = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ success: false, message: 'Error al obtener artículo', error: error?.message });
  }
};
...
export const createArticulo = async (req: Request, res: Response) => {
  try {
    ...
    if (!data.categoria || !data.articulo) {
      return res.status(400).json({ success: false, message: 'Categoría y artículo son requeridos' });
    }
    ...
    return res.status(500).json({ success: false, message: 'Error al crear artículo', error: error?.message });
  }
};
...
export const updateArticulo = async (req: Request, res: Response) => {
  try {
    ...
    if (!updated) {
      return res.status(404).json({ success: false, message: 'Artículo no encontrado o sin cambios' });
    }

    return res.json({ success: true, message: 'Artículo actualizado correctamente' });
  } catch (error: any) {
    console.error('Error en updateArticulo:', error);
    return res.status(500).json({ success: false, message: 'Error al actualizar artículo', error: error?.message });
  }
};
...
export const deleteArticulo = async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ success: true, message: 'Artículo eliminado correctamente' });
  } catch (error: any) {
    console.error('Error en deleteArticulo:', error);
    return res.status(500).json({ success: false, message: 'Error al eliminar artículo', error: error?.message });
  }
};
  ...
  try {
    ...
    if (!data || !uniqueKey || !uniqueKey.categoria || !uniqueKey.articulo) {
      return res.status(400).json({
        success: false,
        message: 'Faltan datos requeridos: data, uniqueKey.categoria, uniqueKey.articulo'
      });
    }

    console.log(`[SYNC ROW] Procesando fila ${row || '?'} de hoja ${sheet || '?'}:`, {
    ...
    const articuloData: any = {
      ...
      pct_ganancia_pretendida: toSafeNumberOrNull(data.PCT_GANANCIA_PRETENDIDA || data.pct_ganancia_pretendida),
      precio_sugerido: toSafeNumberOrNull(data.PRECIO_SUGERIDO || data.precio_sugerido),
      ganancia_final: toSafeNumberOrNull(data.GANANCIA_FINAL || data.ganancia_final),
      pct_ganancia_final: toSafeNumberOrNull(data.PCT_GANANCIA_FINAL || data.pct_ganancia_final),
      disponibilidad: (data.DISPONIBILIDAD === 'DESHABILITADO' || data.disponibilidad === 'DESHABILITADO') 
        ? 'DESHABILITADO' 
        : 'HABILITADO',
      fecha_alta_producto: toMySQLDateTimeArgentina(data.FECHA_ALTA_PRODUCTO || data.fecha_alta_producto) || null,
      fecha_actualizacion_costo: toMySQLDateTimeArgentina(data.FECHA_ACTUALIZACION_COSTO || data.fecha_actualizacion_costo) || null,
      fecha_actualizacion_producto: toMySQLDateTimeArgentina(data.FECHA_ACTUALIZACION_PRODUCTO || data.fecha_actualizacion_producto) || null,
      tipo_carta: data.TIPO_CARTA || data.tipo_carta || null,
    };

    console.log(`[SYNC ROW] Datos normalizados:`, articuloData);
    console.log(`[SYNC ROW] Buscando registro con: categoria="${uniqueKey.categoria}", articulo="${uniqueKey.articulo}"`);
